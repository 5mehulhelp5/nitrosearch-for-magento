 ...
 *
 * The signature below is not this module's output recorded back as an
 * expectation — that would only prove the module agrees with itself, and a
 * canonicalisation that drifts on both sides at once is exactly the shape this
 * has to catch. It is the vector the SERVICE's verifier and the WooCommerce
 * connector both reproduce, so a change to any one signer shows up here as a
 * mismatch rather than in production as an unexplained 401.
 *
 * The inputs are synthetic throughout — `ns_sec_test`, `supersecret`,
 * `shop.test` — and exist only to pin a hash. Nothing here is a credential.
 *
 * REGENERATE ONLY AS A DELIBERATE, COORDINATED WIRE-CONTRACT CHANGE. Every
 * connector and the service move together, or the ones that did not move stop
 * being able to send.
 */

require_once dirname(dirname(__DIR__)).'/lib/Support/Hmac.php';
...
return array(
    ...
    'the module signs its own canonical string to the same value' => function () {
    ...
    'headers() sends exactly the names the service reads' => function () {
        ...
        // name cannot be recovered, so the request fails 401 with nothing in it
        // to say why. Pinned as an exact set, in order, so a rename on either
        // side lands here.
        $headers = Hmac::headers(NS_KEY_ID, NS_SECRET, 'POST', '/v1/ingest/batch', NS_BODY, 'https://shop.test', 'install-xyz');

        ns_is('header names', array(
            'X-NS-Key',
            'X-NS-Timestamp',
            'X-NS-Jti',
            'X-NS-Signature',
        ...
        // THE SECRET IS NEVER A HEADER. It is the signing key; a module that sent
